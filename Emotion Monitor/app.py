...
_last_client_ts_ms = 0

@app.route("/api/process_frame", methods=["POST"])
def api_process_frame():
    global _last_client_ts_ms
    try:
        import base64
        ...
        if now_ms <= _last_client_ts_ms:
            now_ms = _last_client_ts_ms + 33
        _last_client_ts_ms = now_ms
        ...
        print("[ERROR] process_frame:", e, flush=True)
        return jsonify(monitor.get_metrics())
